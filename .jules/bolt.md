...
## 2026-05-21 - Fast-path HTML tag rejection in `filterReadable`
**Learning:** In hot text processing paths, executing a regex replacement like `htmlTagRe.ReplaceAllString(s, "")` on strings that do not contain HTML tags still incurs high CPU overhead and unnecessary string allocations due to the regex state machine. By adding a simple boolean fast-path using `strings.IndexByte(s, '<') >= 0 && strings.IndexByte(s, '>') >= 0` and branching logic, we bypass the regex execution entirely for clean text. This completely eliminates regex-related heap allocations (from 3 allocs down to 0 allocs per call) and improves baseline parsing speed significantly (e.g., from ~1580 ns/op down to ~1220 ns/op). Furthermore, swapping single-character string checks like `strings.Contains(s, "{")` to `strings.IndexByte(s, '{') >= 0` reduces baseline lookup overhead further.
**Action:** When applying regex cleanup inside hot loops, always try to implement a cheap O(n) pre-check (like `strings.IndexByte` for crucial boundary characters) to conditionally bypass the regex entirely when it isn't needed.

## 2026-05-24 - Bypassing Cascadia Selectors in Hot Loops
**Learning:** Using `cascadia.Match()` in hot loops (like evaluating every `<a>` tag's ancestors during HTML scraping) introduces huge performance bottlenecks due to complex parsing overhead and struct allocations, even with pre-compiled matchers.
**Action:** Replace `cascadia.Match()` calls inside hot loops with manual traversals of the `golang.org/x/net/html` tree. Directly inspect `.Data` and `.Attr` fields. For checking classes against multiple tokens, implement a zero-allocation token scanner instead of `strings.Contains` or `regexp`. This yielded an 85%+ improvement in execution time for `linkInArticleOrMain`.

## 2026-05-25 - Eliminating `goquery.Selection.Find` for Element Extraction (`internal/crawler/colly.go`)
**Learning:** In functions that discover links across large DOM subtrees (`buildResultFromMainHTMLWithDoc` and `collectScrapeLinks`), using `goquery.Selection.Find("a[href]").Each(...)` invokes the Cascadia CSS selector engine and allocates multiple temporary `goquery.Selection` slices. By bypassing GoQuery and manually traversing the underlying `golang.org/x/net/html` tree using `FirstChild` and `NextSibling` checks, CPU overhead is nearly halved and numerous heap allocations are eliminated, directly accelerating link extraction in hot loops.
**Action:** When extracting a specific set of tags (like all `<a>` links) from a DOM element or subtree without complex matching logic, replace `goquery.Find("a[href]")` with manual recursive `html.Node` traversals and verify attributes using loop scans.

## 2026-05-26 - Eliminating `goquery.Selection.Find("script")` for Javascript Evaluation (`internal/extractor/jseval.go`)
**Learning:** In `ExtractJsDataFromHTML`, iterating through the parsed DOM to discover and extract `<script>` tag contents using `goquery.Find("script").Each(...)` invokes the Cascadia CSS selector engine and allocates memory for multiple GoQuery structures. While not the most intensive path compared to DOM scoring, bypassing GoQuery by manually walking the `x/net/html` tree and pulling out specific attributes and text drops allocations significantly (e.g. from 53 allocs to 6) and dramatically increases baseline processing speed (e.g. from 12000 ns/op down to 2400 ns/op, an 80% improvement).
**Action:** When extracting a specific set of tags (like all `<script>` tags) from a DOM without complex matching requirements, avoid using `goquery.Find()`. Instead, perform manual recursive traversals of the underlying `html.Node` structures to save heap allocations and Cascadia overhead.

## 2026-05-27 - Fast-path Metadata Extraction in DOM Scanning
**Learning:** During HTML scraping (`internal/crawler/colly.go`), calling `e.DOM.Find("title").Text()` along with `.Find("meta[name='description']")` and `.Find("html")` leverages GoQuery and Cascadia to parse the entire DOM and allocate slices for matching. By implementing a manual recursive `html.Node` traversal (`extractMetadataFast`), we can extract all three metadata values in a single pass without any struct allocations. Furthermore, because metadata tags are almost exclusively located in the `<head>`, we can significantly speed up the extraction by aborting the traversal immediately when encountering the `<body>` tag, effectively pruning the largest portion of the document tree.
**Action:** When extracting standard metadata tags from a full HTML document, use a manual `html.Node` traversal combined with an early exit condition on the `<body>` tag to bypass unnecessary scanning of the entire document, rather than relying on multiple `goquery.Find()` calls.
...
## 2026-05-30 - Eliminating Regex Allocation for HTML Strip (`internal/extractor/jseval.go`)
**Learning:** In the text filtering path (`filterReadable`), standard Go regex replacement (`regexp.MustCompile("<[^>]+>").ReplaceAllString(s, "")`) incurs extremely heavy struct allocation and state machine overhead even for simple boundary parsing. By replacing regex execution with a zero-allocation `strings.Builder` using a manual character scanning logic `stripHTMLTagsStrict`, heap allocations are virtually eliminated and the function becomes significantly faster (e.g., benchmark execution dropping from ~1780 ns/op down to ~412 ns/op, an almost 4.3x speedup).
**Action:** Do not use `regexp.ReplaceAllString` to perform simple repetitive substring or tag strippings inside hot data evaluation loops. Use `strings.Builder` and manual byte iteration (with pre-allocated bounds `.Grow(len(s))`) to execute structural string stripping without causing unnecessary GC pressure.

## 2026-06-06 - Manual html.Node Traversals for Single Tag Lookups
**Learning:** In functions that attempt to find the first instance of a specific tag (like `RecoverMarkdownH1` looking for the first `<h1>`), using `doc.Find("h1").First()` is highly inefficient. It initializes Cascadia matchers and iterates over the DOM, allocating `goquery.Selection` objects. Replacing it with a manual `html.Node` traversal (`walk`) that immediately returns upon finding the target tag reduces execution time by more than half (~30255 ns/op down to ~14214 ns/op) and eliminates the heavy allocation overhead associated with GoQuery's selector parsing.
**Action:** When extracting a single specific tag (e.g., `h1`) from a parsed DOM, prefer manual recursive `html.Node` traversals that allow for early exits over using `goquery.Find("tag").First()`.
...
## 2026-07-04 - Direct string equality for HTML attributes
**Learning:** The standard library `golang.org/x/net/html` automatically lowercases all element tags and attribute keys upon parsing the document. Using case-insensitive comparisons like `strings.EqualFold(a.Key, "role")` against attribute keys introduces unnecessary overhead (type checks and byte iteration) compared to a direct equality check `a.Key == "role"`. The benchmark reveals replacing `strings.EqualFold` with `==` on pre-lowercased node properties like `.Data` or `.Attr` yields a ~10% performance improvement in hot loops.
**Action:** When manually traversing the `x/net/html` tree and inspecting nodes, always use direct string equality (`==`) for tags (`n.Data`) and attribute keys (`a.Key`). Only use `strings.EqualFold` or similar methods for attribute *values* (`a.Val`), which are not automatically lowercased during parsing.
