...
**Action:** Manually traverse the underlying `x/net/html` tree recursively (using `FirstChild` and `NextSibling`) to completely eliminate temporary struct/slice allocations when collecting nodes.
