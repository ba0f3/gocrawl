...
## 2026-04-12 - Replacing `strings.ToLower` + `strings.Contains` with Zero-Allocation Matcher
**Learning:** In a web crawler, frequently scanning large chunks of HTML (up to 256KB) for structural markers or challenge snippets using `strings.ToLower(html)` followed by `strings.Contains` creates enormous GC pressure by repeatedly copying large strings onto the heap purely for case-insensitive matching.
**Action:** Extract the custom zero-allocation substring matcher (previously hidden in `internal/extractor/score.go` as `hasScorePattern`) into a public `utils.HasAnyLowercasePattern(s, patterns)`. This function provides a fast-path ASCII byte matching directly against the target string, bypassing the need to ever lowercase the parent string. Always ensure the `patterns` passed to this utility are pre-lowercased, and apply it globally for string pattern discovery in hot paths.
