...
## 2024-06-24 - Do not manually reimplement unicode.Is* functions
**Learning:** Reimplementing `unicode.IsLetter`, `unicode.IsSpace`, and `unicode.IsPunct` with manual ASCII character range checks for micro-optimizations degrades readability significantly and can introduce subtle logic bugs (e.g. missing `\v` for spaces or misclassifying math symbols as punctuation). Go's standard library functions are already highly optimized with inlineable O(1) table lookups. The only valid optimization here was early return based on `len(s)` before `strings.TrimSpace`.
**Action:** Always prefer standard library `unicode` functions over manual character range checks, and avoid micro-optimizations that heavily degrade readability unless profiling shows a specific, unavoidable bottleneck where stdlib overhead is proven to be the issue.
