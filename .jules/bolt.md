...
## 2025-05-29 - Manual HTML Traversal vs GoQuery For Metadata
**Learning:** Extracting metadata (`<title>`, `<meta name="description">`, `<html lang="...">`) using `goquery` in hot scraping loops incurs massive CPU and memory overhead due to multi-selector parsing, Cascadia matching, and allocating new struct wrappers.
**Action:** Replace `e.DOM.Find(...).Text()` and `.AttrOr(...)` with a manual, zero-allocation recursive walk over `x/net/html` nodes (`e.DOM.Nodes`). Ensure to heavily optimize it by instantly returning when the `<body`> tag is encountered, entirely bypassing traversal for the bulk of the document body since metadata is primarily contained in the `<head>`. Use zero-allocation helper functions (or inline logic) like `strings.EqualFold` for case-insensitive attribute checks.
