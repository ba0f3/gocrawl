...
## 2026-06-26 - Early Bounds Length Check Before Trimming String (`internal/extractor/jseval.go`)
**Learning:** In hot loops mapping over string payloads (like filtering strings using `filterReadable`), evaluating length constraints (like `>= 15`) *after* performing `strings.TrimSpace(s)` executes full loop scans for spaces across the entire bounds of strings that will eventually be dropped. Since trimming can never *increase* length, checking the length beforehand and immediately returning bypasses all byte-scanning and overhead completely for small strings.
**Action:** When filtering short strings or imposing length requirements on strings to be formatted, apply the simplest and cheapest logic (e.g. `len(s) <= threshold`) first, bypassing all downstream parsing operations.
